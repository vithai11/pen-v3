...
st.title("⚽ AI Dự Đoán Hướng Sút - Random Forest Tối Ưu")

# Khởi tạo session
if "kick_history" not in st.session_state:
    st.session_state.kick_history = deque(maxlen=100)  # Sử dụng deque để dễ dàng lấy 3 lượt gần nhất
...
def smart_kick_rf():
    ...
    # Lấy 3 lượt gần nhất
    last_kick = st.session_state.kick_history[-3:]
    last_jump = st.session_state.goalie_jump_history[-3:]
    input_feature = np.array([last_kick + last_jump]).reshape(1, -1)  # Đảm bảo đúng kích thước đầu vào
...
if st.session_state.pending_direction:
    ...
    if goalie_dir:
        ...
        # Huấn luyện model nếu đủ dữ liệu (3 lượt gần nhất)
        if len(st.session_state.kick_history) >= 3:
            X, y = [], []
            kicks = list(st.session_state.kick_history)
            jumps = list(st.session_state.goalie_jump_history)
            for i in range(2, len(kicks)):
                X.append([kicks[i-2], kicks[i-1], kicks[i], jumps[i-2], jumps[i-1], jumps[i]])  # 3 lượt gần nhất
                y.append(jumps[i])
            st.session_state.model.fit(X, y)

        # Gợi ý cho lượt sau
        st.session_state.ai_suggestion, st.session_state.probs = smart_kick_rf()
...
if st.session_state.ai_suggestion:
    ...
    if st.session_state.probs is not None:
        ...
        st.write(f"🔹 Trái: {st.session_state.probs[0] * 100:.2f}%")
        st.write(f"🔹 Giữa: {st.session_state.probs[1] * 100:.2f}%")
        st.write(f"🔹 Phải: {st.session_state.probs[2] * 100:.2f}%")
